<script>
  ...
  const addModule = async () => {
    const result = await fetch('modules/addModule.json', {
      method: 'POST',
      headers: {
...
<Dialog persistent bind:active>
  <div class="dialog_content">
    ...
    >
    <Card>
      <svelte:fragment slot="card_title">Add new module</svelte:fragment>
      <form slot="card_body">
        <TextField rules={moduleIDrules} dense outlined class="mb-4"
          >Module ID</TextField
        >
        <TextField rules={moduleNamerules} dense outlined>Module Name</TextField
        >
      </form>
      <div slot="card_footer" class="button_wrapper">
        ...
        <Button
          ...
          class={!inputs.module_id.isValid || !inputs.module_name.isValid
            ? ''
            : 'primary-color'}
          on:click={() => addModule()}>Add Module</Button
        >
      </div>
    </Card>
  </div>
...
<Snackbar
  ...
  timeout={2000}
>
  {error === false ? 'Module Added Successfuly' : 'Something went wrong...'}
</Snackbar>
